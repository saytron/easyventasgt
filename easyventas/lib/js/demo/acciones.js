...
async function obtener_detalleProducto(Codigo, descripcion, precio, codigo_id_producto, rol, idventaProducto, pass, identificador) {
	...
	var arg_rol = rol;
	var arg_idventaproducto = idventaProducto;
	var arg_identificador = identificador;
	arg_suma = 0;
...
function eliminar_venta(iddetalle, idproducto, precio, compra, idventa) {
...
function eliminarVenta() {
	...
	var url = "Controller/eliminarVentaCotizacion.php";
...
async function generarFactura(codigo) {
	...
	const datos = {
	  ...
	  const respuesta = await respuestaRaw.json();
	  	let uuid = 				respuesta['uuid'];
		let certifier = 		respuesta['certifier']['name'];
		let taxCode = 			respuesta['certifier']['tax_code'];
		let invoiceUrl = 		respuesta['invoice_url'];
		let satSerie = 			respuesta['sat']['serie'];
		let satNo = 			respuesta['sat']['no'];
		let satAuthorization = 	respuesta['sat']['authorization'];
		let certificationDate = respuesta['sat']['certification_date'];
		console.log(email);
...
/*
function chartMarca() {
  var ctx = document.getElementById('ventasMarca')
  var myChart = new Chart(ctx, {
	  type: 'polarArea',
	  data: {
