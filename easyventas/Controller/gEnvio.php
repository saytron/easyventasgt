   ...
   if($datos != ""){
     foreach ($datos as $fila) {
           $precioUnitario=$fila['precio_venta'] / $fila['cantidad'];
      	 	$pdf->Row(array($fila['cantidad'],$fila['codigo']. ' '.$fila['descripcion2'],' Q '.number_format($precioUnitario,2),' Q '.number_format($fila['precio_venta'],2)));  
     }
	}
  $pdf->Ln(3);
  $totalVenta = 0;
 ...
 $estado = 0;
	 if($datos != ""){
        ...
        if($fila['estado'] == 0){
          ...
          $pdf->SetFont('Arial','',10);
          $pdf->Cell(160,6,utf8_decode('ABONO EN FECHA '.$fechaFormato2),0,0,'R');
